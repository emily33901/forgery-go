...
type GLFW struct {
	...
	keyPressedMap map[rune]int
}

// NewGLFW attempts to initialize a GLFW context.
func NewGLFW(io imgui.IO, title string, img image.Image) (*GLFW, error) {
	...
	platform.keyPressedMap = make(map[rune]int)
...
func (platform *GLFW) PostRender() {
	...
	// Update the keypressed maps
	// We do this here becuase processEvents is called before NewFrame()
	// and doing this in NewFrame() would result in losing the magic 1 & -1 values
	for k := range platform.keyPressedMap {
		if platform.keyPressedMap[k] > 0 {
			platform.keyPressedMap[k]++
		} else {
			platform.keyPressedMap[k]--
		}
	}
...
func (platform *GLFW) keyChange(window *glfw.Window, key glfw.Key, scancode int, action glfw.Action, mods glfw.ModifierKey) {
	if action == glfw.Press {
		...
		platform.keyPressedMap[rune(int(key))] = 1
	}
...
func (platform *GLFW) IsKeyPressed(key rune) bool {
...
func (platform *GLFW) SetCursorDisabled(state bool) {
	if state == true {
		platform.window.SetInputMode(glfw.CursorMode, glfw.CursorDisabled)
	} else {
		platform.window.SetInputMode(glfw.CursorMode, glfw.CursorNormal)
	}
}

func (platform *GLFW) FrameCount() int {
